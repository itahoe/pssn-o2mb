/**
  * @file    bsp_nvm.c
  * @brief   Board Support Package - NVM - EEPROM Emulation
...
extern  uint16_t        VirtAddVarTab[ SYS_NVM_ADDR_MAX ];     //Virtual address defined by the user: 0xFFFF value is prohibited
                                        ...
                                        uint16_t        Data )
{
        ...
        for( VarIdx = 0; VarIdx < SYS_NVM_ADDR_MAX; VarIdx++ )
        {
                if( VirtAddVarTab[VarIdx] != VirtAddress )  /* Check each variable except the one passed as parameter */
                {
                        /* Read the other last variable updates */
                        ReadStatus = bsp_nvm_read16(VirtAddVarTab[VarIdx], &DataVar);
        ...
        switch( PageStatus0 )
        {
                ...
                case RECEIVE_DATA:
                        if (PageStatus1 == VALID_PAGE) /* Page0 receive, Page1 valid */
                        {
                                ...
                                for( idx = 0; idx < SYS_NVM_ADDR_MAX; idx++)
                                {
                                        if (( *(__IO uint16_t*)(PAGE0_BASE_ADDRESS + 6)) == VirtAddVarTab[ idx ])
                                        {
                                                x       = idx;
                                        }
                                        ...
                                        if( idx != x )
                                        {
                                                ...
                                                ReadStatus = bsp_nvm_read16( VirtAddVarTab[ idx ], &DataVar );

                                                /* In case variable corresponding to the virtual address was found */
                                                if (ReadStatus != 0x1)
                                                {
                                                        /* Transfer the variable to the Page0 */
                                                        EepromStatus = EE_VerifyPageFullWriteVariable( VirtAddVarTab[ idx ], DataVar );
                ...
                case VALID_PAGE:
                        ...
                        else /* Page0 valid, Page1 receive */
                        {
                                ...
                                for( idx = 0; idx < SYS_NVM_ADDR_MAX; idx++ )
                                {
                                        if((*(__IO uint16_t*)(PAGE1_BASE_ADDRESS + 6)) == VirtAddVarTab[ idx ] )
                                        {
                                                x       = idx;
                                        }
